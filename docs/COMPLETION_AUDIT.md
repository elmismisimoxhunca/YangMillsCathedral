...
| Dimension 2 consistency | Kinematic layer; gauge-fixed holonomy nucleus; selected-loop Haar-density law; same-density convolution semigroup; retained rigorous 2D source chain | Plaquette probes; exact restriction/covariance; closed positive-area pushforward; derived expectation; all-positive-time normalization, `x⁻¹*g` convolution, weak identity limit, and derived half-area split | Partial | The density is explicitly not a heat kernel: connected Lie/metric/Laplacian/Brownian/heat-equation data are absent. No rigorous measure or planar loop geometry is constructed; general face/refinement products, compact-surface gluing, and lattice convergence remain absent; no 2D result may inhabit 4D |
...
7. **Dimension contracts:** the 2D gauge-fixed nucleus, selected-loop normalized-Haar density
   ...
   metric heat-kernel semantics, planar geometry, general face/refinement, projective/gluing, and
