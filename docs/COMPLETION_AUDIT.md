...
| Dimension 2 consistency | Kinematic/gauge-fixed/selected-loop layers; normalized-Haar semigroup; pairing Laplacian; heat equation; Brownian realization; smooth bi-invariant Lie-group Riemannian metric; rigorous 2D sources | Exact restriction/expectation; normalization/convolution/weak identity; `∂ₜQ=½ΔQ`; continuous independent-increment process and selected-loop coherence; Maurer–Cartan inverse, positive pairing, `Ad(h⁻¹)` right transport, bi-invariance, finite-dimensional von Neumann-bounded unit ellipsoids, centerwise smooth tangent-coordinate Maurer coefficients, exact nested Hom-coordinate reconciliation, and packaged `ContMDiffRiemannianMetric` | Partial | No density, solution, process, Laplace–Beltrami comparison, rigorous YM measure, or planar loop geometry is constructed; process measurability is fixed-time. General face/refinement products, compact-surface gluing, and lattice convergence remain absent; no 2D result may inhabit 4D |
