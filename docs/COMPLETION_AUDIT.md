...
| Interpreted gauge-invariant observables | `CurvatureSquaredInterpretation.lean`, `CurvaturePowerInterpretation.lean`, exact `1`, `F²`, `(F²)²` core fields; exact pulled-chain interpretation transport | Interpretation, quartic anti-collapse, gauge-transport, and principal-connection probes | Partial | An independent quantum gauge-action interface and broader-observable gauge invariance, all independent/mixed contractions, covariant derivatives, and a complete local-observable grammar remain open |
