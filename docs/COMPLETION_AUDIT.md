...
**Last fully validated implementation commit:** `33ec5e2` (`cathedral`)
...
| Dimension 2 consistency | Kinematic/gauge-fixed/selected-loop layers; normalized-Haar semigroup; pairing Laplacian; heat equation; Brownian realization; smooth bi-invariant Lie-group Riemannian metric; finite oriented-edge/product-Haar/face-weight infrastructure; concrete uninhabited simple-boundary topological planar-graph certificate; rigorous 2D sources | Exact restriction/expectation; normalization/convolution/weak identity; `∂ₜQ=½ΔQ`; continuous independent-increment process and selected-loop coherence; Maurer–Cartan inverse, positive pairing, `Ad(h⁻¹)` right transport, bi-invariance, finite-dimensional von Neumann-bounded unit ellipsoids, centerwise smooth tangent-coordinate Maurer coefficients, exact nested Hom-coordinate reconciliation, packaged `ContMDiffRiemannianMetric`, and one-coordinate/reversal/append-order edge-word probes, exact open/closed endpoint gauge covariance, normalized finite product Haar, exact marginals, product gauge invariance, measurable finite-word density products, the generic weighted carrier, and normalized finite vertical/`C¹`-horizontal admissibility strengthening, embedded-trace/disjoint-complement/x-axis-cell/once-around-frontier/coordinate-area geometry, and doubled-circuit probes | Partial | No density, solution, process, Laplace–Beltrami comparison, rigorous YM measure, or planar loop geometry is constructed; process measurability is fixed-time. General face/refinement products, compact-surface gluing, and lattice convergence remain absent; no 2D result may inhabit 4D |
...
| Hostile probes | 322 `*Probes.lean` files for 651 Lean files at baseline; all imported major probes are rooted through `YangMills.lean` | Full build and namespace audit | Partial but broad | No verifier proves that every semantic requirement has an adequate mutation probe; some support modules have no same-name probe and require indirect-coverage review |
...
| No `sorry` or project axioms | `scripts/audit_lean.py`; `YangMills/Audit.lean`; root import graph | Source audit passed 651 Lean files; kernel audit passed 8,723 declarations | Implemented at baseline | Must rerun at final commit; result covers imported `YangMills` declarations, not physical adequacy |
...
Evidence inspected at validated implementation commit `33ec5e2`:

| Command or gate | Baseline result | What it verifies | What it does not verify |
|---|---:|---|---|
| `lake build YangMills` | PASS, 3,831 jobs | Elaboration, compilation, all root imports, and execution of the root audit command | Source fidelity, completeness, consistency/inhabitation, or the final objective |
| Kernel namespace audit in `YangMills.lean` | PASS, 8,723 declarations | No transitive unexpected axioms/`sorryAx` in imported `YangMills` declarations under the audit policy | Unimported files, semantic adequacy, or literature interpretation |
| `python3 scripts/audit_lean.py` | PASS, 651 Lean files | Conservative source scan and confirmation that semantic audit is rooted | Mathematical correctness or source completeness |
| `python3 scripts/verify_sources.py` at `33ec5e2` | PASS, 39 manifests | Retained native artifacts, all 31 reproducible native extractions, and the two recursive Mathpix/adjudication artifact trees match their manifests | This does not establish authority, interpretation, redistribution safety, or declaration sufficiency |
...
| `git status --short` | Empty immediately after `33ec5e2` | The inspected documented state through that follow-up commit had no pending changes | Later implementation work or remote publication |
