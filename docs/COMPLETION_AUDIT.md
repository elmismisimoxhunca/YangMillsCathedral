...
| Dimension 2 consistency | Kinematic/gauge-fixed/selected-loop layers; normalized-Haar semigroup; pairing Laplacian; heat equation; Brownian realization; pointwise bi-invariant metric pairing; rigorous 2D sources | Exact restriction/expectation; normalization/convolution/weak identity; `∂ₜQ=½ΔQ`; continuous independent-increment process and selected-loop coherence; Maurer–Cartan inverse, positive pairing, `Ad(h⁻¹)` right transport and bi-invariance | Partial | No density, solution, process, smooth Riemannian metric, Laplace–Beltrami comparison, rigorous YM measure, or planar loop geometry is constructed; process measurability is fixed-time. General face/refinement products, compact-surface gluing, and lattice convergence remain absent; no 2D result may inhabit 4D |
