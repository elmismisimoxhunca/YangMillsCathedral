...
**Last fully validated implementation commit:** `a5d8563` (`cathedral`)

**Baseline working tree immediately after that commit:** clean

**Subsequent supplied-source ingestion working tree:** independently validated where explicitly
marked below; not attributed to `a5d8563` until the ingestion commit is recorded
...
| Dimension 2 consistency | `Dimensions/TwoDimensionalNondegeneracy.lean`; retained Driver/Gross–King–Sengupta/Sengupta/Lévy/Witten/Atiyah–Bott source chain | Nonzero area, spacelike tests, plaquette holonomy and positive density probes; complete acquisition/role audit with selected load-bearing locators | Partial | The acquisition blocker is closed, but no source-specific rigorous continuum probability/holonomy record or witness is yet formalized; no 2D result may inhabit the 4D endpoint |
...
| Hostile probes | 299 `*Probes.lean` files for 603 Lean files at baseline; all imported major probes are rooted through `YangMills.lean` | Full build and namespace audit | Partial but broad | No verifier proves that every semantic requirement has an adequate mutation probe; some support modules have no same-name probe and require indirect-coverage review |
...
| No `sorry` or project axioms | `scripts/audit_lean.py`; `YangMills/Audit.lean`; root import graph | Source audit passed 603 Lean files; kernel audit passed 7,780 declarations | Implemented at baseline | Must rerun at final commit; result covers imported `YangMills` declarations, not physical adequacy |
| Declaration-level provenance | Baseline: `docs/SOURCE_MAP.md`, `docs/PROVENANCE.md`, 13 manifests. Supplied-source working tree: `docs/SUPPLIED_SOURCE_INGESTION_AUDIT.md` and 39 manifests | Baseline source verifier plus working-tree supplied-bundle deduplication, Mathpix selftest/adjudication, and manual source-map inspection | Partial | The 39-manifest result is working-tree evidence, not evidence at `a5d8563`; a manifest verifies bytes only; exact load-bearing locators, redistribution rights, completeness and interpretation still require final row-by-row audit |
...
| Small verified commits | Git history through `a5d8563` | Recent commits have targeted/full builds and audits recorded in status | Implemented locally | Publication requirement remains blocked |
...
Evidence inspected at validated implementation commit `a5d8563`:

| Command or gate | Baseline result | What it verifies | What it does not verify |
|---|---:|---|---|
| `lake build` | PASS, 3,766 jobs | Elaboration, compilation, all root imports, and execution of the root audit command | Source fidelity, completeness, consistency/inhabitation, or the final objective |
| Kernel namespace audit in `YangMills.lean` | PASS, 7,780 declarations | No transitive unexpected axioms/`sorryAx` in imported `YangMills` declarations under the audit policy | Unimported files, semantic adequacy, or literature interpretation |
| `python3 scripts/audit_lean.py` | PASS, 603 Lean files | Conservative source scan and confirmation that semantic audit is rooted | Mathematical correctness or source completeness |
| `python3 scripts/verify_sources.py` at baseline `a5d8563` | PASS, 13 manifests | Baseline retained artifact bytes match manifests | That a source is authoritative, correctly read, or sufficient for a declaration |
| `python3 scripts/verify_sources.py` on supplied-source ingestion working tree | PASS, 39 manifests | Retained native artifacts, all 31 reproducible native extractions, and the two recursive Mathpix/adjudication artifact trees match their manifests | This working-tree result is not attributed to `a5d8563`; it does not establish authority, interpretation, redistribution safety, or declaration sufficiency |
...
| `git status --short` | Empty immediately after `a5d8563` | The inspected documented state through that follow-up commit had no pending changes | Later implementation work or remote publication |
...
7. **Dimension contracts:** rigorous 2D literature is now retained and audited but its probability/
   holonomy interfaces are not formalized; 3D/4D remain `CurrentStrength`; witness-level
   lower-dimensional separation awaits the final proposition.
