...
| Freed p. 8 §1; text 394–398 | adjoint bundle `g_P = P ×_G g`; the right `Ad(g⁻¹)` diagonal convention is induced by (1.12) | Reusable set-level definition / explicit quotient convention | `YangMills.Geometry.adjointBundleRightAction`, `adjointBundleRightAction_one`, `adjointBundleRightAction_mul`, `adjointBundleOrbitRelation`, `adjointBundleSetoid`, `AdjointBundle`, `AdjointBundle.mk`, `AdjointBundle.projection`, `AdjointBundle.mk_rightAction`, `AdjointBundle.mk_eq_mk_iff` | Orbit quotient is formed from the actual principal bundle and actual gauge group. Its projection is well-defined and equality is characterized by an exact orbit witness. Topology, smooth vector-bundle structure, sections/forms, and curvature descent remain pending |
