...
| Freed p. 8 §1; text 394–398 | adjoint bundle `g_P = P ×_G g`; the right `Ad(g⁻¹)` diagonal convention is induced by (1.12) | Reusable set-level definition / explicit quotient convention; project topology decision and derived topology theorems; conditional reusable-mathematics certificate / open debt | `YangMills.Geometry.adjointBundleRightAction`, `adjointBundleRightAction_one`, `adjointBundleRightAction_mul`, `adjointBundleOrbitRelation`, `adjointBundleSetoid`, `AdjointBundle`, `AdjointBundle.mk`, `AdjointBundle.projection`, `AdjointBundle.mk_rightAction`, `AdjointBundle.mk_eq_mk_iff`; Mathlib's generic quotient topology; `AdjointBundle.mk_isQuotientMap`, `.mk_continuous`, `.projection_continuous`, `.projection_surjective`, `.projection_isQuotientMap`; `adjointBundleChartRepresentative`, `adjointBundleChartRepresentative_invariant`, `AdjointBundle.localCoordinate`, `.localCoordinate_mk`, `.localCoordinateInverse`, `.localCoordinate_fst`, `.localCoordinateInverse_localCoordinate`, `.localCoordinate_localCoordinateInverse`; `lieGroupAdjointCoordinates`, `ContinuousLieGroupAdjointData`, `.action_continuous`, `.inverseAction_continuous`; `AdjointBundle.localSource`, `.localTarget`, `.localCoordinate_continuousOn`, `.localCoordinateInverse_continuousOn`, `.localTrivialization` | Orbit quotient is formed from the actual principal bundle and actual gauge group. Its projection is well-defined, equality is characterized by an exact orbit witness, the total topology is the representative quotient topology, and the associated projection is a continuous quotient map onto the declared base. Each principal chart induces representative-independent coordinates `(π(p), Ad(k)X)` with proved local inverse laws. Conditional on the explicitly isolated continuity certificate for `g ↦ Ad(g)` in model coordinates, these maps package as an open partial homeomorphism over `baseSet`; no certificate inhabitant is supplied. A reusable proof of that parameter-dependent derivative fact, smooth vector-bundle structure, sections/forms, and curvature descent remain pending |
