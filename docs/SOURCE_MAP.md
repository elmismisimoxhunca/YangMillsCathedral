...
| Freed p. 8 §1; text 394–398 | adjoint bundle `g_P = P ×_G g`; the right `Ad(g⁻¹)` diagonal convention is induced by (1.12) | Reusable set-level definition / explicit quotient convention; project topology decision and derived topology theorems | `YangMills.Geometry.adjointBundleRightAction`, `adjointBundleRightAction_one`, `adjointBundleRightAction_mul`, `adjointBundleOrbitRelation`, `adjointBundleSetoid`, `AdjointBundle`, `AdjointBundle.mk`, `AdjointBundle.projection`, `AdjointBundle.mk_rightAction`, `AdjointBundle.mk_eq_mk_iff`; Mathlib's generic quotient topology; `AdjointBundle.mk_isQuotientMap`, `.mk_continuous`, `.projection_continuous`, `.projection_surjective`, `.projection_isQuotientMap`; `adjointBundleChartRepresentative`, `adjointBundleChartRepresentative_invariant`, `AdjointBundle.localCoordinate`, `.localCoordinate_mk`, `.localCoordinateInverse`, `.localCoordinate_fst`, `.localCoordinateInverse_localCoordinate`, `.localCoordinate_localCoordinateInverse` | Orbit quotient is formed from the actual principal bundle and actual gauge group. Its projection is well-defined, equality is characterized by an exact orbit witness, the total topology is the representative quotient topology, and the associated projection is a continuous quotient map onto the declared base. Each principal chart now induces representative-independent set-level coordinates `(π(p), Ad(k)X)` with proved local inverse laws. Continuity/local-homeomorphism packaging, smooth vector-bundle structure, sections/forms, and curvature descent remain pending |
