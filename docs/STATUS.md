...
- The living audit is now anchored to fully validated implementation commit `f38e7ef` on branch
  `cathedral`: 3,617 build jobs, 6,686 kernel-audited declarations, 454 Lean files, a then-clean
...
sign reversal, and the non-inverted adjoint factor when distinguishable. Smooth bundling and the
Maurer–Cartan structure equation, descended active covariance, action invariance, and observable
gauge invariance remain open.
