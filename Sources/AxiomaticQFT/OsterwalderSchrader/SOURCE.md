...
`SameFieldOSIIOutputCorrelatorUniquenessData` remains a narrow supporting correlator-extension
obligation. The stronger uninhabited `CorrectedOSIIReconstructionAcceptanceData` requires selected
`(R0′)`, relative analytic, and exact-source Wick data and quantifies, within the selected Hilbert
universe, over every alternative Wightman output on the same exact Poincaré lift carrying those
corrected hypotheses for the same Euclidean family. Each alternative must be fixed-lift unitarily
equivalent in the Streater–Wightman p. 118 sense and have the same full tempered
distributions. This is acceptance semantics, not a proof or construction of reconstruction.
