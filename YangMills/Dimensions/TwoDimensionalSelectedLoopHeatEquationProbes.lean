...
The probes expose the compact Lie-group heat core without a compact-simple index, its positive smooth real representative,
...
/-- The heat-equation core is available at compact Lie-group scope without a compact-simple datum;
connectedness is imposed later by Driver's common chain. -/
theorem exact_compact_lie_heat_core
    (data : TwoDimensionalSelectedLoopHeatEquationCoreData
      inner law semigroup laplacian) :
    data.densityReal = data.densityReal :=
  rfl
