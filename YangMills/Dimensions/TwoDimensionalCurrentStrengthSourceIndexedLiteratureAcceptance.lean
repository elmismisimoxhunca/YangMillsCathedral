...
The unchanged proposition is deliberately named `CurrentStrength`. It is not yet the final
source-complete 2D target. A separate strict record below adds the fixed-universe universal general
Fact 3 finite-curve-law acceptance on the exact witness selected by current strength. Unrestricted
Facts 2–3, heat-factor integration, analytic and stochastic inhabitation, and several source bridges
remain open. No inhabitant is constructed.
-/
...
/-- Strict fixed-universe strengthening of current strength by the universal general Fact 3
finite-curve-law acceptance on the exact embedded Sengupta base, finite law, and covering density
selected by one unchanged current-strength witness. -/
structure TwoDimensionalGeneralFactThreeCurrentStrengthSourceIndexedLiteratureAcceptanceData where
  ...
  generalFactThree :
    TwoDimensionalSenguptaUniversalGeneralHomeomorphismFiniteCurveLawData.{uG, uCover,
      ...
      uSenguptaSurface, uSenguptaBaseVertex, uGeneralTargetEdge, uGeneralTargetInternal,
      uGeneralTargetFace, uGeneralTargetRegion, uGeneralTargetSurface, uGeneralTargetVertex,
      uGeneralSourceFineEdge, uGeneralSourceFineInternal, uGeneralSourceFineFace,
      uGeneralSourceFineVertex, uGeneralTargetFineEdge, uGeneralTargetFineInternal,
      uGeneralTargetFineFace, uGeneralTargetFineVertex}
...
/-- Proposition-shaped fixed-universe general-Fact-3 strengthening of current strength. It is
nonfinal and packages no inhabitant. -/
def TwoDimensionalGeneralFactThreeCurrentStrengthSourceIndexedLiteratureAcceptance : Prop :=
  Nonempty
...
namespace TwoDimensionalGeneralFactThreeCurrentStrengthSourceIndexedLiteratureAcceptanceData

omit [T2Space CoverGroup] [Nonempty CurveS] [Fintype SenguptaTargetEdge]
    [MeasurableMul₂ CoverGroup] [MeasurableInv CoverGroup] in
/-- Forget the fixed-universe general Fact 3 strengthening to unchanged current strength. -/
theorem implies_current
    (data : TwoDimensionalGeneralFactThreeCurrentStrengthSourceIndexedLiteratureAcceptanceData.{uE,
...
omit [T2Space CoverGroup] [Nonempty CurveS] [Fintype SenguptaTargetEdge]
    [MeasurableMul₂ CoverGroup] [MeasurableInv CoverGroup] in
/-- The stronger source join retains the exact rank-two compact-surface model. -/
theorem model_finrank_two
    (data : TwoDimensionalGeneralFactThreeCurrentStrengthSourceIndexedLiteratureAcceptanceData.{uE,
      ...
      (SenguptaFineVertex := SenguptaFineVertex) (senguptaFine := senguptaFine)
      (SenguptaTargetVertex := SenguptaTargetVertex) (senguptaTarget := senguptaTarget)
      (SenguptaTargetSurface := SenguptaTargetSurface)) : Module.finrank ℝ EG = 2 :=
  (Classical.choice data.current).compactSurface_model_finrank_two

omit [T2Space CoverGroup] [Nonempty CurveS] [Fintype SenguptaTargetEdge]
    [MeasurableMul₂ CoverGroup] [MeasurableInv CoverGroup] in
/-- The stronger source join still rejects a four-dimensional linear model. -/
...
namespace TwoDimensionalGeneralFactThreeSelectedFourierDerivativeCurrentStrengthSourceIndexedLiteratureAcceptanceData

omit [T2Space CoverGroup] [Nonempty CurveS] [Fintype SenguptaTargetEdge]
    [MeasurableMul₂ CoverGroup] [MeasurableInv CoverGroup] in
/-- Derive the all-smooth zero-time stochastic generator on the exact spectral bridge selected by
this same strict general-Fact-3 source witness. This constructs no new analytic input. -/
noncomputable def toStochasticGeneratorAtZeroData
    (data : TwoDimensionalGeneralFactThreeSelectedFourierDerivativeCurrentStrengthSourceIndexedLiteratureAcceptanceData.{uE,
      uG, uGauge, uSample, uConnection, uΩ, uVertex, uEdge, uFace, uXAxisCell,
      uLargeVertex, uLargeEdge, uLargeFace, uLargeXAxisCell, uFineVertex, uFineEdge,
      uFineFace, uFineXAxisCell, uFineLargeVertex, uFineLargeEdge, uFineLargeFace,
      uFineLargeXAxisCell, uEL, uHL, uSL, uER, uHR, uSR, uEG, uHG, uLeftBase,
    ...
    TwoDimensionalSelectedLoopStochasticGeneratorAtZeroData
      ((Classical.choice data.source.current).augmented.current.planar
        |>.toSpectralBrownianGeneratorBridgeData) :=
  data.selectedFourierDerivative.toStochasticGeneratorAtZeroData

end TwoDimensionalGeneralFactThreeSelectedFourierDerivativeCurrentStrengthSourceIndexedLiteratureAcceptanceData
...
namespace TwoDimensionalGeneralFactThreeSelectedFourierDerivativeCurrentStrengthSourceIndexedLiteratureAcceptance

omit [T2Space CoverGroup] [Nonempty CurveS] [Fintype SenguptaTargetEdge]
    [MeasurableMul₂ CoverGroup] [MeasurableInv CoverGroup] in
/-- The exact joint general-Fact-3/Fourier-derivative acceptance implies the existing direct
operator-generator source acceptance. General Fact 3 is forgotten only in the conclusion. -/
theorem implies_operatorGenerator
    (acceptance :
      TwoDimensionalGeneralFactThreeSelectedFourierDerivativeCurrentStrengthSourceIndexedLiteratureAcceptance.{uE,
        uG, uGauge, uSample, uConnection, uΩ, uVertex, uEdge, uFace, uXAxisCell,
        uLargeVertex, uLargeEdge, uLargeFace, uLargeXAxisCell, uFineVertex, uFineEdge,
        uFineFace, uFineXAxisCell, uFineLargeVertex, uFineLargeEdge, uFineLargeFace,
        uFineLargeXAxisCell, uEL, uHL, uSL, uER, uHR, uSR, uEG, uHG, uLeftBase,
        ...
        (SenguptaTargetVertex := SenguptaTargetVertex) (senguptaTarget := senguptaTarget)
        (SenguptaTargetSurface := SenguptaTargetSurface)) :
    TwoDimensionalOperatorGeneratorCurrentStrengthSourceIndexedLiteratureAcceptance.{uE, uG,
      uGauge, uSample, uConnection, uΩ, uVertex, uEdge, uFace, uXAxisCell,
      uLargeVertex, uLargeEdge, uLargeFace, uLargeXAxisCell, uFineVertex,
      uFineEdge, uFineFace, uFineXAxisCell, uFineLargeVertex, uFineLargeEdge,
      uFineLargeFace, uFineLargeXAxisCell, uEL, uHL, uSL, uER, uHR, uSR, uEG, uHG,
      uLeftBase, uRightBase, uWholeBase, uLeftLoop, uRightLoop, uWholeLoop,
      uLeftSample, uRightSample, uWholeSample, uCover, uCurveS, uEdgeS, uInternalEdge,
      uFaceS, uRegionS, uSenguptaSample, uSenguptaSurface, uSenguptaBaseVertex,
      uSenguptaFineInternal, uSenguptaFineFace, uSenguptaFineVertex, uSenguptaTargetEdge,
      uSenguptaTargetInternal, uSenguptaTargetFace, uSenguptaTargetRegion,
      uSenguptaTargetVertex, uSenguptaTargetSurface, uCandidateFineEdge,
      uCandidateFineInternal, uCandidateFineFace, uCandidateFineVertex, uPath, uObservable}
      (law := law) (inner := inner) (realLaplacian := realLaplacian)
      (complexLaplacian := complexLaplacian) (heatTraceData := heatTraceData)
      (continuum := continuum) (faceGeometry := faceGeometry) (Ω := Ω)
      (identification := identification) (IG := IG)
      ...
      (SenguptaTargetSurface := SenguptaTargetSurface) := by
  rcases acceptance with ⟨data⟩
  exact ⟨TwoDimensionalCurrentStrengthSourceIndexedLiteratureComponents.ofStrongest
      (Classical.choice data.source.current),
    ⟨data.toStochasticGeneratorAtZeroData⟩⟩

omit [T2Space CoverGroup] [Nonempty CurveS] [Fintype SenguptaTargetEdge]
    [MeasurableMul₂ CoverGroup] [MeasurableInv CoverGroup] in
/-- Exact inhabitance audit: the joint proposition contains only one strict general-Fact-3 source
and the selected-Fourier derivative obligations dependently indexed by that source's chosen bridge. -/
theorem nonempty_iff_components :
    TwoDimensionalGeneralFactThreeSelectedFourierDerivativeCurrentStrengthSourceIndexedLiteratureAcceptance.{uE,
      uG, uGauge, uSample, uConnection, uΩ, uVertex, uEdge, uFace, uXAxisCell,
      uLargeVertex, uLargeEdge, uLargeFace, uLargeXAxisCell, uFineVertex, uFineEdge,
    ...
    ∃ source :
      TwoDimensionalGeneralFactThreeCurrentStrengthSourceIndexedLiteratureAcceptanceData.{uE, uG,
        ...
        (SenguptaTargetSurface := SenguptaTargetSurface),
      TwoDimensionalSelectedLoopSmoothMatrixCoefficientSelectedFourierDerivativeAnalyticAcceptance
        ((Classical.choice source.current).augmented.current.planar
          |>.toSpectralBrownianGeneratorBridgeData) := by
  constructor
  · rintro ⟨data⟩
    exact ⟨data.source, data.selectedFourierDerivative⟩
  · rintro ⟨source, selectedFourierDerivative⟩
    exact ⟨⟨source, selectedFourierDerivative⟩⟩

end TwoDimensionalGeneralFactThreeSelectedFourierDerivativeCurrentStrengthSourceIndexedLiteratureAcceptance
