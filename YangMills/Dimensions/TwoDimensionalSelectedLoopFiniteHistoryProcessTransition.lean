...
A second theorem uses simultaneous prefix reconstruction to transport arbitrary continuous tests to
the actual finite process-evaluation history. Neither theorem identifies the union of all finite
histories with the full process-past sigma-algebra. No conditional Markov theorem, Brownian process,
or Yang--Mills measure is constructed.
-/
...
omit [FiniteDimensional ℝ E] in
/-- Weak Markov identity for arbitrary continuous tests of the actual finite process-evaluation
history `(B(t₁),…,B(tₙ))`. Prefix-product reconstruction transports the previously proved increment-
history identity to this process-history carrier.

This quantifies over every fixed finite family starting at zero. It is not yet conditioning on the
sigma-algebra generated by all times up to the present. -/
theorem TwoDimensionalSelectedLoopSpectralBrownianGeneratorBridgeData.finiteProcessHistory_weakMarkov_identity
    (bridge : TwoDimensionalSelectedLoopSpectralBrownianGeneratorBridgeData
      (law := law) (inner := inner) (realLaplacian := realLaplacian)
      (complexLaplacian := complexLaplacian) (heatTraceData := heatTraceData) (Ω := Ω))
    (n : ℕ) (times : Fin (n + 2) → NNReal) (times_monotone : Monotone times)
    ...
    ∫ samplePoint,
      ...
      ∂bridge.brownian.probabilityMeasure := by
  let Φ : C(Fin n → G, ℝ) :=
    Ψ.comp (finiteRightIncrementPrefixProductsContinuousMap n)
  have reconstructed :=
    bridge.brownian.pastRightIncrementPrefixProducts_ae_eq_processValues n times times_zero
  have transition :=
    bridge.finiteIncrementHistory_process_weakMarkov_identity
      n times times_monotone times_zero t ht final_time Φ f
  calc
    _ = ∫ samplePoint,
        Φ (twoDimensionalPastRightIncrements bridge.brownian n times samplePoint) *
          f (bridge.brownian.process (times (Fin.last (n + 1))) samplePoint)
      ...
      apply integral_congr_ae
      filter_upwards [reconstructed] with samplePoint hrecon
      simp only [Φ, ContinuousMap.comp_apply]
      rw [hrecon]
    _ = _ := transition
    _ = _ := by
      apply integral_congr_ae
